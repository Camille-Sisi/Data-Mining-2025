  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "commune", 
                  ...
                  choices = unique(OffresPE_2020$location_zipcode),
                  selected = "75013")
      ),
...
server <- function(input, output) {
   ...
   OffresPE_2020_react <- reactive({
    OffresPE_2020 %>% 
      filter(location_departement==input$dept & location_zipcode==input$commune)
   ...
   observe({
     ...
     if(input$dept == "75"){
       updateSelectInput(inputId = "commune",
                         choices = unique(OffresPE_2020[OffresPE_2020$location_departement=="75", ]$location_zipcode),
                         selected = "75013")
      }
     else {updateSelectInput(inputId = "commune",
                            choices = unique(OffresPE_2020[OffresPE_2020$location_departement==input$dept, ]$location_zipcode),
  ...
  output$barplot <- renderPlot({
  OffresPE_2020_react() %>% 
      filter(location_zipcode==input$commune) %>% 
      count(type_contrat) %>% 
      mutate(Pourcentage=round(prop.table(n)*100,1)) %>% 
      select(-n) %>%
